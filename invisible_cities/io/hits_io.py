...
def hitc_from_df(dst : pd.DataFrame, skip_NN : bool = False) -> Dict[int, HitCollection]:
    """
    Utter laziness, just takes the singular event using hits_from_df. PLEASE CHANGE ME
    Function that transforms pandas DataFrame dst to HitCollection
    ------
    Parameters
    ------
    ...
    # if only one key -> one event
    if len(all_events.keys()) == 1:
        return all_events[next(iter(all_events.keys()))]
